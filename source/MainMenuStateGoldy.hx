...
class MainMenuStateGoldy extends MusicBeatState
{
	...
	override function create()
	{
		...
		var menuThing:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menuThing'));
		menuThing.scrollFactor.set(0, yScroll);
		menuThing.setGraphicSize(Std.int(bg.width * 1.175));
	...
	override function update(elapsed:Float)
	{
		if (ClientPrefs.moreShit == 'Ice')
		   MusicBeatState.switchState(new MainMenuStateIce());
		else if (ClientPrefs.moreShit == 'Psych')
			MusicBeatState.switchState(new MainMenuStatePsych());
