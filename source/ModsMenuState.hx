...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		while (i < modsList.length)
		{
			...
			if (loadedIcon != null)
			{
				newMod.icon = new AttachedSprite().loadGraphic(loadedIcon, true, 150, 150);//animated icon support
			...
			{
				newMod.icon = new AttachedSprite().loadGraphic(Paths.image('unknownMod'));
			}
